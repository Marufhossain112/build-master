...
export default function PcBuild({ categoryData }) {
  const selectedData = useSelector(
    (state) => state.persistedPcBuilderReducer.selectedProduct
  );
  // console.log("selected data", selectedData);
  // console.log("categoryData", categoryData);
  const handleCompleteBuild = () =>
    toast("Build is completed successfully", {
  ...
  return (
    <div className="">
      ...
      <div className="flex gap-2 flex-col mx-1">
        {categoryData &&
          categoryData.map((category, index) => (
            <div
              key={index}
              className="parent  border border-gray-400 rounded-sm p-2 w-full  md:w-[50%]  mx-auto flex justify-between overflow-auto"
            >
              {/* category logo and name */}
              <div
                className="flex gap-1 max-w-md max-h-16"
                    ...
                    {product.category === category.name ? (
                      <>
                        <div className="flex items-center  rounded-md">
                          ...
                          <p className="">{product.name.slice(0, 20)}</p>
                          {/* <p>
                                            {product.price}
                                        </p> */}
                        </div>
      ...
      <button
        disabled={selectedData.length < 7}
        onClick={handleCompleteBuild}
        className="btn btn-outline block mx-auto my-3"
      >
        Complete build
      </button>
    </div>
